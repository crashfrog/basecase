...
class TestDummyUrlConf(TestCase):
	...
	def setUp(self):
		Request = collections.namedtuple('Request', ('method',))
	...
	def testJobs(self):
		self.assertEquals([api.dummy.jobs(self.get), api.dummy.jobs(self.get)], self.api_test('jobs/'))
		self.assertEquals(api.dummy.jobs(self.get), self.api_test('jobs/id/1'))

	def testJobsFiles(self):
		self.assertEquals(api.dummy.job_files(self.get), self.api_test('jobs/id/1/files/'))

	def testLogs(self):
		self.assertEquals(api.dummy.logs(self.get), self.api_test('jobs/id/1/logs'))

	def testJobtypes(self):
		self.assertEquals([api.dummy.jobtypes(self.get), api.dummy.jobtypes(self.get)], self.api_test('jobtypes/'))
		self.assertEquals(api.dummy.jobtypes(self.get), self.api_test('jobtypes/id/1'))

	def testAnalyses(self):
		self.assertEquals([api.dummy.analyses(self.get), api.dummy.analyses(self.get)], self.api_test('analyses/'))
		self.assertEquals(api.dummy.analyses(self.get), self.api_test('analyses/id/1'))
